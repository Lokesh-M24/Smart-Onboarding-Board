...
Description: Rotates welcome messages and shows QR codes for onboarding. Auto-rotates slides, clear keyboard controls shown onscreen.
'''
    ...
    def load_content(self):
        ...
        try:
            ...
            if not self.qr_links:
                self.qr_links = [{'title': 'Company LMS', 'url': 'https://www.tcs.com/careers'}]
...
'''
    ...
    def generate_qr_codes(self):
        '''Generate QR code for the first link (main QR code)'''
        if not self.qr_links:
            return
        try:
            qr = qrcode.QRCode(
            ...
            qr.add_data(self.qr_links[0]['url'])
            qr.make(fit=True)
    ...
    def create_slide(self, message_index):
        ...
        # Keyboard controls block
        self.draw_controls(slide)
        return slide
    ...
    def draw_controls(self, img):
        controls = [
            ...
            "n - next slide",
            "p - previous slide",
            "r - reload content.txt",
            "q/Esc  quit"
        ]
        panel_x, panel_y = 50, self.height - 350  # Place above bottom
        padding = 0
        line_height = 38
        ctrl_width = 330
        ctrl_height = line_height * len(controls) + padding * 2
        # Light background block for controls (with slight shadow for clarity)
        cv2.rectangle(img, (panel_x - 2 + 3, panel_y - 2 + 3), 
                      (panel_x + ctrl_width + 2 + 3, panel_y + ctrl_height + 2 + 3), (220, 220, 220), -1)
        cv2.rectangle(img, (panel_x - 2, panel_y - 2), 
                      (panel_x + ctrl_width + 2, panel_y + ctrl_height + 2), (255, 255, 255), -1)
        for i, line in enumerate(controls):
            scale = 0.85 if i == 0 else 0.72
            color = self.header_color if i == 0 else self.text_color
            cv2.putText(img, line, (panel_x, panel_y + i * line_height + 30),
                        self.font, scale, color, 2)
    ...
    def run_display(self):
        ...
        while True:
            ...
            if key == ord('q') or key == 27:  # Quit
                ...
                self.generate_qr_codes()
            # Auto-advance
            if time.time() - last_slide_time >= self.slide_duration:
                self.current_slide = (self.current_slide + 1) % len(self.messages)
                last_slide_time = time.time()
